...
using OtelNLWebBetter.Telemetry;
...
internal class Program
{
    ...
    private static async Task Main(string[] args)
    {
        ...
        var telemetryServiceName = "otelnlwebbetter";
        var otlpEndpoint = new Uri("http://localhost:18889");
        var azureConnectionString = _configuration["Logging:APP_INSIGHTS_CONNECTION"];
        var jsonExportPath = Path.Combine(Environment.CurrentDirectory, "trace-export.jsonl");

        // 🎯 Activity source
        _activitySource = new ActivitySource("Copilot-Agent-Plugins");
        ...
        var loggerFactory = LoggerFactory.Create(builder =>
        {
            builder
                ...
                .AddOpenTelemetry(logging =>
                {
                    ...
                    logging.SetResourceBuilder(ResourceBuilder.CreateDefault().AddService(telemetryServiceName));
        ...
        // 🔐 Graph Auth Handler
        _bearerAuthenticationProviderWithCancellationToken =
            new BearerAuthenticationProviderWithCancellationToken(_configuration, bearerLogger);

        // 📡 Unified Tracing
        Sdk.CreateTracerProviderBuilder()
            .AddSource("Copilot-Agent-Plugins")
            .AddHttpClientInstrumentation()
            .SetResourceBuilder(ResourceBuilder.CreateDefault().AddService(telemetryServiceName))
            .AddOtlpExporter(opt => opt.Endpoint = otlpEndpoint)
            .AddAzureMonitorTraceExporter(opt => opt.ConnectionString = azureConnectionString)
            .AddProcessor(new SimpleActivityExportProcessor(new JsonExporter(jsonExportPath))) // JSONL exporter for NLWeb
            .Build();
